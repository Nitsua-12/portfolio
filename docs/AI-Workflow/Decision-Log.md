...
## 2026-08-05 — Fluid page transitions via Astro's View Transitions

**Decision:** Added Astro's `<ClientRouter />` (from `astro:transitions`) to the shared layout, so clicking between pages crossfades instead of hard-swapping. The ambient background glow and grain texture persist across navigations (`transition:persist`) so their slow drift animation doesn't restart on every click; the nav bar deliberately does **not** persist, since its active-link state is genuinely different per page and persisting it would freeze the highlight on whatever page loaded first.

**Why:** Direct request — "make a transition happen when I click to another page so it looks fluid rather than a page appearing." Astro's static-output architecture already gives each route a real, SEO-friendly HTML page; View Transitions is the layer that makes navigating between them feel like a single continuous app instead of a stack of separate documents, without giving up any of the static-site benefits.

**Real bug caught while verifying this:** the scroll-reveal `<script>` only runs once per real page load, not on every client-side transition — had to rebind its setup to the `astro:page-load` event (which Astro fires after the initial load *and* after every transition) or reveal animations would silently stop working after the first client-side navigation.

**Bigger bug caught in the same pass:** while testing whether the active nav link's color survived a transition, found that it was never rendering correctly at all, transition or not — Tailwind v4 emits its utilities inside `@layer utilities`, and per the CSS Cascade Layers spec, *any* unlayered rule beats *any* layered rule regardless of specificity. `global.css` had a plain `a { color: inherit; }` sitting unlayered, which was silently defeating every `text-[var(--...)]` utility applied to a link across the whole site, not just the nav. Fixed by wrapping the base element resets in `@layer base` so they participate in Tailwind's own layer ordering instead of sitting above it. This had been live and broken since the very first deploy — the visual QA never happened to catch it because dim-text and hover-text colors look similar enough at a glance.
